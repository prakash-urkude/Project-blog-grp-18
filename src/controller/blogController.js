...
try{
    ...
    jwt.verify(token, "grp-18-first-project", function (err, decodedToken) {
 ...
 let decodedToken = req["decodedToken"];
   ...
   if(authorId){
    ...
    if(decodedToken.authorId != authorId){ return res.status(400).send({status:false, error:"unauthorized person"})}
    else filterdata.authorId=authorId
   }else{filterdata.authorId = decodedToken.authorId}
